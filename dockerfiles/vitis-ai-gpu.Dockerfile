...
RUN  sed -i "s#http://security.ubuntu.com#https://mirrors.bit.edu.cn#g" /etc/apt/sources.list
...
RUN apt-get -y install \
    ...
    && wget -e "HTTPS_PROXY=https://172.30.192.1:41091" -O - https://apt.kitware.com/keys/kitware-archive-latest.asc 2>/dev/null | gpg --dearmor - | tee /etc/apt/trusted.gpg.d/kitware.gpg >/dev/null \
...
# Install XRT
RUN wget -e "HTTPS_PROXY=https://172.30.192.1:41091" --progress=dot:mega -O xrt.deb ${XRT_URL} \
    && ls -lhd ./xrt.deb \
    && apt-get update -y \
...
# Install XRM
RUN wget -e "HTTPS_PROXY=https://172.30.192.1:41091" --progress=dot:mega -O xrm.deb ${XRM_URL} \
    && ls -lhd ./xrm.deb \
...
# glog 0.4.0
RUN cd /tmp \
    && wget -e "HTTPS_PROXY=https://172.30.192.1:41091" --progress=dot:mega -O glog.0.4.0.tar.gz https://codeload.github.com/google/glog/tar.gz/v0.4.0 \
...
# protobuf 3.4.0
RUN cd /tmp; wget -e "HTTPS_PROXY=https://172.30.192.1:41091" --progress=dot:mega https://codeload.github.com/google/protobuf/zip/v3.4.0 \
    && unzip v3.4.0 \
    && cd protobuf-3.4.0 \
    && ./autogen.sh \
    && ./configure \
...
RUN export PATH="/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin" \
    ...
    && git config --global http.proxy http://172.30.192.1:41091 \
    && git config --global https.proxy https://172.30.192.1:41091 \
    && cd /tmp; wget -e "HTTPS_PROXY=https://172.30.192.1:41091" --progress=dot:mega https://github.com/opencv/opencv/archive/3.4.3.tar.gz \
...
# gflags 2.2.2
RUN cd /tmp; wget -e "HTTPS_PROXY=https://172.30.192.1:41091" --progress=dot:mega https://github.com/gflags/gflags/archive/v2.2.2.tar.gz \
...
RUN source ~/.bashrc \
    && wget -e "HTTPS_PROXY=https://172.30.192.1:41091" --progress=dot:mega https://github.com/json-c/json-c/archive/json-c-0.15-20200726.tar.gz \
...
RUN cd /tmp \
    && wget -e "HTTPS_PROXY=https://172.30.192.1:41091" --progress=dot:mega https://github.com/conda-forge/miniforge/releases/download/4.10.3-5/Mambaforge-4.10.3-5-Linux-x86_64.sh -O miniconda.sh \
...
RUN cd /scratch/ \
    ...
    && conda install -c https://mirrors.bfsu.edu.cn/anaconda/cloud/conda-forge/ conda-build \
    && python3 -m pip install --upgrade pip wheel setuptools -i https://mirrors.aliyun.com/pypi/simple/\
    && conda env create -f /scratch/vitis-ai-optimizer_darknet.yml \
    && conda env create -f /scratch/vitis-ai-optimizer_caffe.yml \
    && conda env create -f /scratch/vitis-ai-optimizer_pytorch.yml \
    && conda env create -f /scratch/vitis-ai-optimizer_tensorflow.yml \
    && conda env create -f /scratch/vitis-ai-optimizer_tensorflow2.yml \
    && conda env create -f /scratch/vitis-ai-pytorch.yml \
        && conda activate vitis-ai-pytorch \
        && pip install -r /scratch/pip_requirements.txt -i https://mirrors.aliyun.com/pypi/simple/\
    && conda env create -f /scratch/vitis-ai-caffe.yml \
        && conda activate vitis-ai-caffe \
         &&  conda  install --no-update-deps -y pydot pyyaml jupyter ipywidgets \
        dill progressbar2 pytest scikit-learn pandas matplotlib \
        pillow  -c https://mirrors.bfsu.edu.cn/anaconda/cloud/conda-forge/ -c defaults \
        && pip install -r /scratch/pip_requirements.txt -i https://mirrors.aliyun.com/pypi/simple/\
    ...
    && conda env create -f /scratch/vitis-ai-tensorflow2.yml \
        && conda activate vitis-ai-tensorflow2 \
         &&  conda  install --no-update-deps  -y vai_q_tensorflow2_gpu  pydot pyyaml jupyter ipywidgets \
        dill progressbar2 pytest scikit-learn pandas matplotlib \
        pillow  -c https://mirrors.bfsu.edu.cn/anaconda/cloud/conda-forge/ -c defaults \
        && pip install -r /scratch/pip_requirements.txt -i https://mirrors.aliyun.com/pypi/simple/\
        && pip install --ignore-installed tensorflow==2.6  keras==2.6 -i https://mirrors.aliyun.com/pypi/simple/\
        && pip uninstall -y h5py \
       &&  conda install  -y --override-channels --force-reinstall h5py=2.10.0  -c https://mirrors.bfsu.edu.cn/anaconda/cloud/conda-forge/ -c anaconda \
    && conda clean -y --force-pkgs-dirs \
    && rm -fr ~/.cache \
...
ARG CACHEBUST=1
RUN cd /tmp \
    && wget -e "HTTPS_PROXY=https://172.30.192.1:41091" -O libunilog.deb https://www.xilinx.com/bin/public/openDownload?filename=libunilog_1.4.1-r82_amd64.deb \
    && wget -e "HTTPS_PROXY=https://172.30.192.1:41091" -O libtarget-factory.deb https://www.xilinx.com/bin/public/openDownload?filename=libtarget-factory_1.4.1-r85_amd64.deb \
    && wget -e "HTTPS_PROXY=https://172.30.192.1:41091" -O libxir.deb https://www.xilinx.com/bin/public/openDownload?filename=libxir_1.4.1-r91_amd64.deb \
    && wget -e "HTTPS_PROXY=https://172.30.192.1:41091" -O libvart.deb https://www.xilinx.com/bin/public/openDownload?filename=libvart_1.4.1-r130_amd64.deb \
    && wget -e "HTTPS_PROXY=https://172.30.192.1:41091" -O libvitis_ai_library.deb https://www.xilinx.com/bin/public/openDownload?filename=libvitis_ai_library_1.4.1-r114_amd64.deb \
    && wget -e "HTTPS_PROXY=https://172.30.192.1:41091" -O librt-engine.deb https://www.xilinx.com/bin/public/openDownload?filename=librt-engine_1.4.1-r195_amd64.deb \
    && wget -e "HTTPS_PROXY=https://172.30.192.1:41091" -O aks.deb https://www.xilinx.com/bin/public/openDownload?filename=aks_1.4.1-r78_amd64.deb \
